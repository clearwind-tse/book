...
permalink: /pages/8309a5b876fc95hf
categories: 
  - 源码
  - Spring文章
tags: 
...
​​对于这三行代码，可以说是：如果你要用Spring，你就得这么写。就像你要用Mybatis，你就得写各种Mapper接口。
...
1. 遍历扫描路径下的所有Java类，如果发现某个类上存在@Component、@Service等注解，那么Spring就把这个类记录下来，存在一个Map中，比如Map<String, Class>。（**Spring源码中存在类似的这么一个Map，叫做BeanDefinitionMap**）
...
### 2.1 Bean的创建过程(Bean创建的生命周期)
​

那么Spring到底是如何来创建一个Bean的过程如下
...
​
通过最后一步，当Spring根据UserService类来创建一个Bean时：
...
当 Spring调用有参构造方式时，Spring会根据入参的类型和入参的名字去Spring中找Bean对象（以单例Bean为例，Spring会从单例池那个Map中去找）：
